...
class Day11 {
    ...
    class Solver {
        ...
        data class State(
            val current: String,
            val includeDac: Boolean = false,
            val includeFft: Boolean = false
        )

        fun solvePart1(input: List<String>, startDevice: String = "you", mustIncludeDacFft: Boolean = false): Long {
            val deviceMap = input.associate { line ->
                val parts = line.split(": ")
                val device = parts[0]
                val outputs = if (parts.size > 1) parts[1].split(" ") else emptyList()
                device to outputs
            }

            val endDevice = "out"

            val initialState = State(current = startDevice, includeDac = false, includeFft = false)
            val stack = mutableMapOf(initialState to 1L)

            var pathCount = 0L

            while (stack.isNotEmpty()) {
                val state = stack.keys.first()
                val count = stack.remove(state)!!
                val currentDevice = state.current

                if (currentDevice == endDevice) {
                    if (mustIncludeDacFft) {
                        if (state.includeDac && state.includeFft) {
                            pathCount+= count
                        }
                    } else {
                        pathCount+= count
                ...
                val outputs = deviceMap[currentDevice] ?: emptyList()

                val nextStates =
                    outputs.map { output ->
                        State(
                            current = output,
                            includeDac = state.includeDac || output == "dac",
                            includeFft = state.includeFft || output == "fft"
                        )
                    }

                for (nextState in nextStates) {
                    stack[nextState] = (stack[nextState] ?: 0) + count
                }
        ...
        fun solvePart2(input: List<String>): Long {
            return solvePart1(input, "svr", mustIncludeDacFft = true)
        }
    }
...
fun main() {
    val input = Day11::class.java.getResourceAsStream("/day11_sample.txt")!!.bufferedReader().readLines()
    val input2 = Day11::class.java.getResourceAsStream("/day11_sample2.txt")!!.bufferedReader().readLines()
    val realInput = Day11::class.java.getResourceAsStream("/day11.txt")!!.bufferedReader().readLines()
    val solver = Day11.Solver()
    println("Sample Part1")
    val part1 = solver.solvePart1(input)
    println("Part1")
    val realPart1 = solver.solvePart1(realInput)
    println("Sample Part2")
    val part2 = solver.solvePart2(input2)
    println("Part2")
