...
class Day5 {
    ...
    class Solver {
        fun solvePart1(input: List<String>): Long {
            val ranges = input.takeWhile { !it.isBlank() }.map { line ->
                val (start, end) = line.split("-").map { it.toLong() }
                start..end
            }
            val availableIds = input.dropWhile { !it.isBlank() }.drop(1).map { it.toLong() }

            val freshCount = availableIds.count { id ->
                ranges.any { range -> id in range }
            }
            return freshCount.toLong()
        ...
        fun mergeRanges(ranges: List<LongRange>): List<LongRange> {
            ...
            for (i in 1 until sortedRanges.size) {
                ...
                if (currentRange.last >= nextRange.first - 1) {
        ...
        fun solvePart2(input: List<String>): Long {
            ...
            val freshRanges = mergeRanges(ranges)

            val totalFreshCount = freshRanges.sumOf { it.last - it.first + 1 }
...
fun main() {
    val input = Day5::class.java.getResourceAsStream("/day5_sample.txt")!!.bufferedReader().readLines()
    val realInput = Day5::class.java.getResourceAsStream("/day5.txt")!!.bufferedReader().readLines()
    val solver = Day5.Solver()
