...
class Day7 {
    ...
    class Solver {
        fun solvePart1(input: List<String>): Int {
            ...
            val width = map
            var beams = setOf(map[0].indexOf('S'))
            var currentRow = 1
        ...
        data class Timeline(val beam: Int, val n: Long)

        fun solvePart2(input: List<String>): Long {

            val map = input.map { it.toCharArray() }.toTypedArray()

            var timelines = listOf(Timeline(map[0].indexOf('S'), 1L))
            var currentRow = 1
            while (currentRow < map.size) {
                val newTimelines = mutableListOf<Timeline>()
                for (timeline in timelines) {
                    val nextBeamPositions = if (map[currentRow][timeline.beam] == '^') {
                        listOf(timeline.beam - 1, timeline.beam + 1)
                    } else {
                        listOf(timeline.beam)
                    }
                    for (nextBeamPosition in nextBeamPositions) {
                        val alreadyExistsIndex = newTimelines.indexOfFirst { it.beam == nextBeamPosition }
                        if (alreadyExistsIndex == -1) {
                            newTimelines.add(timeline.copy(beam = nextBeamPosition))
                        } else {
                            val existing = newTimelines[alreadyExistsIndex]
                            newTimelines[alreadyExistsIndex] = existing.copy(n = existing.n + timeline.n)
                        }
                    }
...
fun main() {
    val input = Day7::class.java.getResourceAsStream("/day7_sample.txt")!!.bufferedReader().readLines()
    val realInput = Day7::class.java.getResourceAsStream("/day7.txt")!!.bufferedReader().readLines()
    val solver = Day7.Solver()
