import com.google.ortools.Loader.loadNativeLibraries
...
class Day10 {
    ...
    class Solver {
        ...
        data class State(val lights: String, val presses: Int)

        data class Machine(val target: String, val buttons: List<Set<Int>>, val joltage: List<Int>) {

            fun solve(): Int {
                val explored = mutableSetOf<String>()
                val queue = ArrayDeque<State>()
                queue.add(State(".".repeat(target.length), 0))
                ...
                while (queue.isNotEmpty()) {
                    ...
                    for (button in buttons) {
                        val newLights = current.lights.mapIndexed { index, c ->
                            if (index in button) {
                                if (c == '#') '.' else '#'
                            } else {
                                c
                        ...
                        if (newLights !in explored) {
                            explored.add(newLights)
                            queue.add(State(newLights, current.presses + 1))
                        }
                    }
                }
            ...
            fun solveJoltage(): Int {
                ...
                // Create variables: how many times each button is pressed
                val maxClicks = joltage.max().toLong()
                val buttonClicks = Array(numButtons) { j ->
                    model.newIntVar(0, maxClicks, "button-$j")
                }

                // Add constraints: for each counter, the sum of button presses affecting it must equal the target
                for (i in joltage.indices) {
                    val buttonsAffectingCounter = mutableListOf<IntVar>()

                    buttons.forEachIndexed { buttonIdx, button ->
                        if (i in button) {
                            buttonsAffectingCounter.add(buttonClicks[buttonIdx])
                        }
                    }

                    // Sum of all buttons affecting counter i must equal target joltage[i]
                    val coeffs = LongArray(buttonsAffectingCounter.size) { 1L }
                    model.addEquality(
                        LinearExpr.weightedSum(buttonsAffectingCounter.toTypedArray(), coeffs),
                        joltage[i].toLong()
                    )
                }
            ...
            companion object {
                fun fromInputLine(line: String): Machine {
                    ...
                    val targetMatch = targetRegex.find(line) ?: throw IllegalArgumentException("Invalid line: $line")
                    val target = targetMatch.groupValues[1]

                    val buttonMatches = buttonRegex.findAll(line)
                    val buttons = buttonMatches.map { match ->
                        match.groupValues[1].split(",").map { it.toInt() }.toSet()
                    }.toList()

                    val joltageMatch = joltageRegex.find(line) ?: throw IllegalArgumentException("Invalid line: $line")
                    val joltage = joltageMatch.groupValues[1].split(",").map { it.toInt() }
        ...
        fun solvePart1(input: List<String>): Int {
            return input.map { line -> Machine.fromInputLine(line) }.sumOf { machine ->
                machine.solve()
            }
        }
        ...
        fun solvePart2(input: List<String>): Int {
            ...
            return input.map { line -> Machine.fromInputLine(line) }.mapIndexed { index, machine ->
                println("Solving machine ${index + 1}/${input.size}")
                machine.solveJoltage()
            }.sum()
        }
    }
...
fun main() {
    val input = Day10::class.java.getResourceAsStream("/day10_sample.txt")!!.bufferedReader().readLines()
    val realInput = Day10::class.java.getResourceAsStream("/day10.txt")!!.bufferedReader().readLines()
    val solver = Day10.Solver()
    ...
    println("Real Part 2 solved in ${endTime - startTime} ms")
